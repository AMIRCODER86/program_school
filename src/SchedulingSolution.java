...
public class SchedulingSolution {
    ...
    public static String class1[][] = new String[5][3];
    private static HashMap<String, Integer> assigned_hours = new HashMap<String, Integer>();
   private static SchedulingProblem problem = new SchedulingProblem();
    ...
    public static void assignHour(int day, int hour, String lesson){
        try {
            assigned_hours.replace(lesson, assigned_hours.get(lesson)+1);

        }
        catch (Exception e){
            assigned_hours.put(lesson, 0);

        }
        class1[day][hour]=lesson;
    }
    public static ArrayList<String> getListLeson(int day, int hour) {
        ...
        for (int counter_teacher = 0; counter_teacher < 12; counter_teacher++){
            for (int counter_lesson = 0; counter_lesson < 12; counter_lesson++) {
                if (problem.teacher_avaialability[counter_teacher][day][hour].equals(problem.json_niaz.getJSONObject(String.valueOf(counter_lesson)).getString("0"))){
                    try {
                        if ((assigned_hours.get(problem.json_niaz.getJSONObject(String.valueOf(counter_lesson)).getString("0"))) < (Integer.valueOf(problem.json_niaz.getJSONObject(String.valueOf(counter_lesson)).getString("niaz")))) {
                            list_lesson.add(problem.json_niaz.getJSONObject(String.valueOf(counter_lesson)).getString("0"));
                        }
                    }
                    catch (Exception e){
                        assigned_hours.put(problem.json_niaz.getJSONObject(String.valueOf(counter_lesson)).getString("0"), 0);
                        list_lesson.add(problem.json_niaz.getJSONObject(String.valueOf(counter_lesson)).getString("0"));
                    }
                }
            }
